<div class="form-group form-row">
    ...
    <input type="hidden" id="id_order" value="{{$order->id}}">
        ...
        <select class="form-control" id="id_client" name="id_client">
            <option id="id_cliented" value="{{$order->client->id}}">{{$order->client->company_name}}</option>
    ...
    <div class="col-2 offset-2 form-control" style="margin-top: 25px">
      <div class="custom-control custom-checkbox">
        <input type="checkbox" class="custom-control-input" id="recurenceOrder" name="recurenceOrderCheckBox"
         {{ $order->recurrent == 1 ? 'checked' : ''}} >
        <label class="custom-control-label" for="recurenceOrder">Recurring </label>
      </div>
    </div> 
    <div class="col-12" id="date"> 
      <label >Order Data </label>
      <input type="date" class="form-control" id="orderdate" value="{{date('d-m-Y', strtotime($order->order_date))}}" name="orderdate">
    </div> 
</div> 
...
<div id="form_recurence" style="display: none">
    <div class="form-group form-row">
      ...
      <div class="col-3 offset-3">
          ...
          value=" {{$order->recurrent == 0 ? $order->event->events_meta->repeat_interval / 86400 : 1 }}" 
...
{{--     <div class="form-group form-row">
      <div class="col-6">
          ...
          <input type="date" class="form-control" id="repeat_start" value="{{ $order->event->events_meta->repeat_start }} name="repeat_start">
      </div>

      <div class="col-6">
          <label>repeat end</label>
          <input type="date" class="form-control" id="repeat_end" value="{{ $order->event->events_meta->repeat_end }} name="repeat_end">
      </div>
  </div> --}}
  ...
  <div class="form-group form-row">
      ...
      <div class="col-6 ">
          ...
          <select class="custom-select" id="daysofweek" size="3" multiple>
              <option {{ $order->daysofweek == 'MO' ? 'selected' : ''}} value="MO">Monday</option>
              <option {{ $order->daysofweek == 'TU' ? 'selected' : ''}} value="TU">Tuesday</option>
              <option {{ $order->daysofweek == 'WE' ? 'selected' : ''}} value="WE">Wednesday</option>
              <option {{ $order->daysofweek == 'TH' ? 'selected' : ''}} value="TH">Thursday</option>
              <option {{ $order->daysofweek == 'FR' ? 'selected' : ''}} value="FR">Friday</option>
              <option {{ $order->daysofweek == 'SA' ? 'selected' : ''}} value="SA">Saturday</option>
              <option {{ $order->daysofweek == 'SU' ? 'selected' : ''}} value="SU">Sunday</option>
          </select>
      </div>
  </div>
   <hr>
